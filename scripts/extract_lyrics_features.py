...
def main():
    ...
    for _, row in tqdm(frame.iterrows(), total=len(frame), desc="lyrics"):
        ...
        for i, chunk in enumerate(chunks):
            encoded = tokenizer.prepare_for_model(chunk, add_special_tokens=True, truncation=True, max_length=args.chunk_tokens + tokenizer.num_special_tokens_to_add(), return_tensors="pt")
            encoded = {k: v.to(device) for k, v in encoded.items()}
            with torch.inference_mode():
                out = model(**encoded, return_dict=True)
                mask = encoded.get("attention_mask", torch.ones(out.last_hidden_state.shape[:2], device=device)).unsqueeze(-1)
                pooled = (out.last_hidden_state * mask).sum(dim=1) / mask.sum(dim=1).clamp_min(1)
            song_features[i] = pooled.squeeze(0).cpu()
            song_mask[i] = True
        ids.append(str(row["lyric_song"])); features.append(song_features); masks.append(song_mask)
    output = PROJECT_ROOT / args.output
    output.parent.mkdir(parents=True, exist_ok=True)
    torch.save({"ids": ids, "features": torch.stack(features), "mask": torch.stack(masks), "model": args.model}, output)
    print("Saved", output, torch.stack(features).shape)
